    ...
    module ActiveRecordMacro
      ...
      class_methods do
        def has_one_state_machine(field_name, state_machine_klass:, transition_klass:, transition_name: transition_klass.to_s.underscore.pluralize.to_sym)
          state_machine_name = "#{field_name.to_s}_state_machine"
          ...
          include(const_set("#{field_name.to_s}#{SecureRandom.hex(4)}_mod".classify, Module.new).tap do |mod|
            mod.module_eval do
              extend ActiveSupport::Concern
              ...
              class_methods do
                define_method :"#{field_name}_human_wrapper" do
                  ...
                  instance_variable_set(key, state_machine_klass.constantize.send(:states).map { |s| [I18n.t(s, scope: "statesman.#{field_name}_#{base_klass.underscore}"), s] })
          ...
          reflection = ActiveRecord::Reflection.create(
            ...
            name,
            nil,
            { state_machine_klass: state_machine_klass, transition_klass: transition_klass, transition_name: transition_name },
            self
          )

          yield reflection if block_given?

          ActiveRecord::Reflection.add_state_machine_reflection(self, name, reflection)
        end
      end
    end
  end
